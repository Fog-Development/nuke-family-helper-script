// Detect if this script is running inside the TornPDA mobile app and, if so,
// inject a polyfill for the missing Greasemonkey functions. TornPDA does not
// natively provide the GM_* APIs that browsers like Tampermonkey and
// Greasemonkey expose. To enable scripts to run unmodified, we copy the
// GMforPDA implementation here. This block runs only when the scriptHandler
// indicates TornPDA and the GM functions have not already been defined.
//
// This module MUST be imported first in main.js so the polyfill is installed
// before any other module touches a GM_* function.
export const isPda = (() => {
  try {
    return (
      !!window.GM_info &&
